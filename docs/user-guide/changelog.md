...
- Added `AcceleratorNDArray` building blocks and basic arithmetic ops, plus a GPU `sum` kernel, moving NuMojo towards device-agnostic arrays. [PR #359] [PR #369] [PR #372] - Note that this is an experimental struct aimed at building a device agnostic array, this will be renamed to `NDArray` in future once all the API's have been settled. 
...
- **Removed the `Matrix` type.** All matrix-only routines (`qr`, `eig`, `lstsq`, standalone matrix `transpose`) were removed; the equivalent `NDArray`-based routines (`matmul`, `inv`, `solve`, `lu_decomposition`, `det`, `trace`, `sort`/`argsort`/`sum` with `axis`) remain. Update any code that imported `numojo.Matrix` to use `NDArray` instead. [PR #381]. If you strictly need to perform only 2D `Matrix` computations, please use our other library [linamo](https://github.com/mojomath/linamo).
- Updated the codebase to Mojo v1.0.0 (from v1.0.0b1/b2), including a memory model refactor. [PR #349] [PR #358] [PR #370] [PR #378]
...
- Fixed unsafe pointer access across `NDArray`, `NDArrayShape`, `NDArrayStrides`, and `Item`. [PR #379] [PR #380] - This removes any trace of `MutAnyOrigin` that can trigger the ASAP destruction mode and cause pointer free issues. All the pointer access through `unsafe_ptr` carry correct origin to prevent such memory issues. 
...
### 📚 Documentatory and testing in v0.9.0
...
### 📚 Documentatory and testing v0.8.0
...
### 📚 Documentatory and testing in v0.6.0
