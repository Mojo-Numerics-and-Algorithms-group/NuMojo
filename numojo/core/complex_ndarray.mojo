...
struct ComplexNDArray[
    ...
    fn __init__(
        ...
        ```mojo
        import numojo as nm
        var A = nm.ComplexNDArray[cf32, f32](Shape(2,3,4))
        ```
        """
    ...
    fn __init__(
        ...
        """
        self.re.__init__(ndim, offset, size, shape, strides, coefficient, order)
        self.im.__init__(ndim, offset, size, shape, strides, coefficient, order)

    # ! this could be problematic since we have two pointers to the same data
    fn __init__(
        mut self,
        data_re: UnsafePointer[SIMD[dtype, 1]],
        data_im: UnsafePointer[SIMD[dtype, 1]],
        ndim: Int,
        offset: Int,
        shape: List[Int],
        strides: List[Int],
        ...
        """
        self.re.__init__(
            data_re, ndim, offset, shape, strides, coefficient, order
        )
        self.im.__init__(
            data_im, ndim, offset, shape, strides, coefficient, order
        )
    ...
    fn set(self, owned index: Int, val: ComplexSIMD[cdtype, dtype, 1]) raises:
        ...
        if (index >= self.re.size) or (index < 0):
            raise Error(
    ...
    fn _setitem(self, *indices: Int, val: ComplexSIMD[cdtype, dtype, 1]):
        ...
        for i in range(self.re.ndim):
            index_of_buffer += indices[i] * self.re.strides._buf[i]

        self.re._buf[index_of_buffer] = val.re
        self.im._buf[index_of_buffer] = val.im
    ...
    fn __eq__(
        ...
        """
        return comparison.equal[dtype](self.re, other.re) and comparision.equal[
            dtype
        ](self.im, other.im)
    ...
    fn write_to[W: Writer](self, mut writer: W):
        try:
            writer.write(
                ...
                + str(self.re.ndim)
                + "-D array  "
                + self.re.shape.__str__()
                + "  DType: "
                + self.re.dtype.__str__()
                + "  order: "
                + self.re.order
            )
        except e:
    ...
    fn __repr__(self) -> String:
        ...
        try:
            ...
            if self.re.size > 6:
                for i in range(6):
                    result = result + str(self.get(i)) + str(",")
                result = result + " ... "
            else:
...
### ------------------------------------------------------------------- ###
### COMPLEX NDARRAY DIFFERENT METHOD
### ------------------------------------------------------------------- ###


struct ComplexNDArrayy[
    cdtype: CDType = CDType.float64,
    dtype: DType = CDType.to_dtype[cdtype](),
](Stringable, Representable, CollectionElement, Sized, Writable):
    """The N-dimensional array (NDArray).

    Parameters:
        cdtype: Type of item in NDArray. Default type is DType.float64.
    ...
    fn __init__(
        ...
        self._buf = UnsafePointer[ComplexSIMD[cdtype, 1]]().alloc(self.size)
        self.datatype = cdtype
        self.order = order
    ...
    fn __init__(
        mut self,
        data: UnsafePointer[ComplexSIMD[cdtype, 1]],
        ndim: Int,
        offset: Int,
        shape: List[Int],
        strides: List[Int],
    ...
    fn set(self, owned index: Int, val: ComplexSIMD[cdtype, 1]) raises:
        """
        Safely retrieve i-th item from the underlying buffer.
    ...
    fn _setitem(self, *indices: Int, val: ComplexSIMD[cdtype, 1]):
        """
        (UNSAFE! for internal use only.)
        Get item at indices and bypass all boundary checks.
    ...
    fn __setitem__(mut self, idx: Int, val: ComplexNDArrayy[cdtype]) raises:
        """
        ...
        _traverse_iterative_setter[cdtype](
            val, self, nshape, ncoefficients, nstrides, noffset, index
        )

    fn __setitem__(mut self, index: Idx, val: ComplexSIMD[cdtype, 1]) raises:
        """
    ...
    fn __setitem__(
        mut self, mask: NDArray[DType.bool], value: ComplexSIMD[cdtype, 1]
    ) raises:
        """
    ...
    fn __setitem__(
        mut self, owned *slices: Slice, val: ComplexNDArrayy[cdtype]
    ) raises:
        """
    ...
    fn __setitem__(
        mut self, owned slices: List[Slice], val: ComplexNDArrayy[cdtype]
    ) raises:
        """
    ...
    # fn __setitem__(self, owned *slices: Variant[Slice, Int], val: NDArray[cdtype]) raises:
    #     """
    ...
    fn __setitem__(self, index: NDArray[DType.index], val: NDArray) raises:
        ...
        for i in range(len(index)):
            self.set(
                int(index.get(i)), rebind[ComplexSIMD[cdtype, 1]](val.get(i))
            )

    fn __setitem__(
        mut self, mask: NDArray[DType.bool], val: ComplexNDArrayy[cdtype]
    ) raises:
        """
        Set the value of the array at the indices where the mask is true.
    ...
    # Getter dunders and other getter methods
    # ===-------------------------------------------------------------------===#
    fn get(self, owned index: Int) raises -> ComplexSIMD[cdtype, 1]:
        """
        Safely retrieve i-th item from the underlying buffer.
    ...
    fn _getitem(self, *indices: Int) -> ComplexSIMD[cdtype, 1]:
        """
        (UNSAFE! for internal use only.)
        Get item at indices and bypass all boundary checks.
    ...
    fn __getitem__(self, index: Idx) raises -> ComplexSIMD[cdtype, 1]:
        """
        Set the value at the index list.
        """
        if index.__len__() != self.ndim:
    ...
    fn __getitem__(self, index: List[Int]) raises -> Self:
        ...
        var result = NDArray[cdtype](ndshape)
    ...
    fn __neg__(self) raises -> Self:
        ...
        return self * ComplexSIMD[cdtype, 1](-1.0)
    ...
    fn __eq__(self, other: Self) raises -> NDArray[DType.bool]:
        ...
        Itemwise equivelence.
        """
        return comparison.equal[cdtype](self, other)

    @always_inline("nodebug")
    fn __eq__(
        self, other: ComplexSIMD[cdtype, 1]
    ) raises -> NDArray[DType.bool]:
        """
        Itemwise equivelence between scalar and Array.
        """
    ...
    fn __ne__(
        ...
        """
        return comparison.not_equal[cdtype](self, other)

    @always_inline("nodebug")
    fn __ne__(self, other: NDArray[cdtype]) raises -> NDArray[DType.bool]:
        """
    ...
    fn __lt__(
        ...
        Itemwise less than.
        """
        return comparison.less[cdtype](self, other)

    @always_inline("nodebug")
    fn __lt__(self, other: NDArray[cdtype]) raises -> NDArray[DType.bool]:
        """
    ...
    fn __le__(
        ...
        """
        return comparison.less_equal[cdtype](self, other)

    @always_inline("nodebug")
    fn __le__(self, other: NDArray[cdtype]) raises -> NDArray[DType.bool]:
        """
    ...
    fn __gt__(
        ...
        """
        return comparison.greater[cdtype](self, other)

    @always_inline("nodebug")
    fn __gt__(self, other: NDArray[cdtype]) raises -> NDArray[DType.bool]:
        """
    ...
    fn __ge__(
        ...
        """
        return comparison.greater_equal[cdtype](self, other)

    @always_inline("nodebug")
    fn __ge__(self, other: NDArray[cdtype]) raises -> NDArray[DType.bool]:
        """
        Itemwise less than or equal to between scalar and Array.
        """
        return comparison.greater_equal[cdtype](self, other)
    ...
    fn __add__[
        OtherDType: DType,
        ResultDType: DType = TypeCoercion.result_type[cdtype, OtherDType](),
    ...
    fn __add__(self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `array + scalar`.
        """
        return math.add[cdtype](self, other)
    ...
    fn __radd__(mut self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `scalar + array`.
        """
        return math.add[cdtype](self, other)
    ...
    fn __iadd__(mut self, other: ComplexSIMD[cdtype, 1]) raises:
        """
        Enables `array += scalar`.
        """
    ...
    fn __iadd__(mut self, other: Self) raises:
        ...
        Enables `array *= array`.
        """
        self = _af.math_func_2_array_in_one_array_out[cdtype, SIMD.__add__](
            self, other
        )
    ...
    fn __sub__(self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `array - scalar`.
        """
        return math.sub[cdtype](self, other)
    ...
    fn __rsub__(mut self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `scalar - array`.
        """
        return math.sub[cdtype](other, self)

    fn __isub__(mut self, other: ComplexSIMD[cdtype, 1]) raises:
        """
        Enables `array -= scalar`.
        """
        self = self - other
    ...
    fn __mul__(self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `array * scalar`.
        """
        return math.mul[cdtype](self, other)
    ...
    fn __rmul__(mut self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `scalar * array`.
        """
        return math.mul[cdtype](self, other)

    fn __imul__(mut self, other: ComplexSIMD[cdtype, 1]) raises:
        """
        Enables `array *= scalar`.
        """
        self = self * other
    ...
    fn __invert__(self) raises -> Self:
        ...
        """
        return bitwise.invert[cdtype](self)
    ...
    fn __truediv__(self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `array / scalar`.
        """
        return math.div[cdtype](self, other)
    ...
    fn __itruediv__(mut self, s: ComplexSIMD[cdtype, 1]) raises:
        """
        Enables `array /= scalar`.
        """
    ...
    fn __rtruediv__(self, s: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `scalar / array`.
        """
        return math.div[cdtype](s, self)
    ...
    fn __floordiv__(self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `array // scalar`.
        """
        return math.floor_div[cdtype](self, other)
    ...
    fn __ifloordiv__(mut self, s: ComplexSIMD[cdtype, 1]) raises:
        """
        Enables `array //= scalar`.
        """
    ...
    fn __rfloordiv__(self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `scalar // array`.
        """
        return math.floor_div[cdtype](other, self)
    ...
    fn __mod__(mut self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `array % scalar`.
        """
        return math.mod[cdtype](self, other)

    fn __mod__(mut self, other: NDArray[cdtype]) raises -> Self:
        """
        Enables `array % array`.
        """
        return math.mod[cdtype](self, other)

    fn __imod__(mut self, other: ComplexSIMD[cdtype, 1]) raises:
        """
        Enables `array %= scalar`.
        """
        self = math.mod[cdtype](self, other)

    fn __imod__(mut self, other: NDArray[cdtype]) raises:
        """
        Enables `array %= array`.
        """
        self = math.mod[cdtype](self, other)

    fn __rmod__(mut self, other: ComplexSIMD[cdtype, 1]) raises -> Self:
        """
        Enables `scalar % array`.
        """
        return math.mod[cdtype](other, self)
    ...
    fn __repr__(self) -> String:
        ...
        """
        try:
            var result: String = str("NDArray[DType.") + str(self.cdtype) + str(
                "](List[Scalar[DType."
            ) + str(self.cdtype) + str("]](")
            if self.size > 6:
                for i in range(6):
    ...
    fn __iter__(self) raises -> _NDArrayIter[__origin_of(self), cdtype]:
        """Iterate over elements of the NDArray, returning copied value.
        ...
        return _NDArrayIter[__origin_of(self), cdtype](
            array=self,
            length=self.shape[0],
        )
    ...
    fn __reversed__(
        self,
    ) raises -> _NDArrayIter[__origin_of(self), cdtype, forward=False]:
        """Iterate backwards over elements of the NDArray, returning
        copied value.
        ...
        return _NDArrayIter[__origin_of(self), cdtype, forward=False](
            array=self,
            length=self.shape[0],
        )
    ...
    fn vdot(self, other: Self) raises -> ComplexSIMD[cdtype, 1]:
        """
        ...
        var sum = ComplexSIMD[cdtype, 1](0)
    ...
    # * We fix return cdtype of this array shape for the linalg solve module.
    ...
    fn load[width: Int = 1](self, index: Int) -> SIMD[cdtype, width]:
        """
    ...
    fn load[width: Int = 1](self, *index: Int) raises -> SIMD[cdtype, width]:
        """
    ...
    fn store[width: Int](mut self, index: Int, val: SIMD[cdtype, width]):
        """
    ...
    fn store[
        width: Int = 1
    ](mut self, *index: Int, val: SIMD[cdtype, width]) raises:
        """
    ...
    fn all(self) raises -> Bool:
        ...
        if not (self.cdtype is DType.bool or self.cdtype.is_integral()):
    ...
    fn argmax(self) -> Int:
        ...
        """
        var result: Int = 0
        var max_val: ComplexSIMD[cdtype, 1] = self.load[width=1](0)
        for i in range(1, self.size):
            var temp: ComplexSIMD[cdtype, 1] = self.load[width=1](i)
            if temp > max_val:
                max_val = temp
                result = i
        return result
    ...
    fn argmin(self) -> Int:
        ...
        """
        var result: Int = 0
        var min_val: ComplexSIMD[cdtype, 1] = self.load[width=1](0)
        for i in range(1, self.size):
            var temp: ComplexSIMD[cdtype, 1] = self.load[width=1](i)
            if temp < min_val:
                min_val = temp
    ...
    fn cumprod(self) raises -> NDArray[cdtype]:
        """
        ...
        """
        return cumprod[cdtype](self)

    fn cumprod(self, axis: Int) raises -> NDArray[cdtype]:
        """
        ...
        """
        return cumprod[cdtype](self, axis=axis)

    fn cumsum(self) raises -> NDArray[cdtype]:
        """
        ...
        """
        return cumsum[cdtype](self)

    fn cumsum(self, axis: Int) raises -> NDArray[cdtype]:
        """
        ...
        """
        return cumsum[cdtype](self, axis=axis)

    fn diagonal(self):
        pass

    fn fill(mut self, val: ComplexSIMD[cdtype, 1]):
        """
    ...
    fn item(self, *index: Int) raises -> ComplexSIMD[cdtype, 1]:
        """
        ...
        Example:
        ```console
        > var A = nm.NDArray[cdtype](3, 3, random=True, order="F")
        > print(A)
        [[      14      -4      -48     ]
        [      97      112     -40     ]
        [      -59     -94     66      ]]
        ...
        Returns:
            A scalar matching the cdtype of the array.
        """
    ...
    fn itemset(
        mut self, index: Variant[Int, List[Int]], item: ComplexSIMD[cdtype, 1]
    ) raises:
    ...
    fn max(self, axis: Int = 0) raises -> Self:
        ...
        var result: NDArray[cdtype] = self[slices]
        for i in range(1, axis_size):
            ...
            result = arithmetic.add(
                result * bool_to_numeric[cdtype](mask2),
                arr_slice * bool_to_numeric[cdtype](mask1),
            )
    ...
    fn mean(self) raises -> ComplexSIMD[cdtype, 1]:
        """
        Cumulative mean of a array.

        Returns:
            The cumulative mean of the array as a SIMD Value of `cdtype`.
        """
        return cummean[cdtype](self)

    # fn nonzero(self):
    #     pass

    fn prod(self: Self) raises -> ComplexSIMD[cdtype, 1]:
        """
    ...
    fn round(self) raises -> Self:
        ...
        Returns:
            An NDArray.
        """
        return rounding.tround[cdtype](self)
    ...
    fn sum(self: Self) raises -> ComplexSIMD[cdtype, 1]:
        """
    ...
    fn tolist(self) -> List[ComplexSIMD[cdtype, 1]]:
        """
        ...
        Returns:
            A 1-D List.
        """
        var result: List[ComplexSIMD[cdtype, 1]] = List[
            ComplexSIMD[cdtype, 1]
        ]()
        for i in range(self.size):
    ...
    fn trace(
        ...
        """
        return linalg.norms.trace[cdtype](self, offset, axis1, axis2)
    ...
    fn reshape(self, shape: NDArrayShape, order: String = "C") raises -> Self:
        ...
        """
        return reshape[cdtype](self, shape=shape, order=order)
    ...
    fn unsafe_ptr(self) -> UnsafePointer[ComplexSIMD[cdtype, 1]]:
        """
...
#         is_mutable: Whether the iterator is mutable.
#         origin: The lifetime of the underlying NDArray data.
#         cdtype: The data type of the item.
#         forward: The iteration direction. `False` is backwards.
#     """

#     var index: Int
#     var array: NDArray[cdtype]
#     var length: Int

#     fn __init__(
#         mut self,
#         array: NDArray[cdtype],
#         length: Int,
#     ):
#         self.index = 0 if forward else length
#         self.length = length
...
#     fn __next__(mut self) raises -> NDArray[cdtype]:
#         @parameter
#         if forward:
#             var current_index = self.index
#             self.index += 1
