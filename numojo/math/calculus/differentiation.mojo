...
fn gradient[
    ...
    var result: NDArray[out_dtype] = NDArray[out_dtype](x.shape(), random=False)
    var space: NDArray[out_dtype] =  numojo.arange[in_dtype, out_dtype](1, x.num_elements() + 1, step=spacing.cast[in_dtype]())
    ...
    result[x.num_elements()-1] = (x[x.num_elements() - 1].cast[out_dtype]() - x[x.num_elements() - 2].cast[out_dtype]()) / (hu - hd)

    for i in range(1, x.num_elements()-1):
        var hu: Scalar[out_dtype] = space[i + 1] - space[i]
        var hd: Scalar[out_dtype] = space[i] - space[i - 1]
        var fi: Scalar[out_dtype] = (hd**2 * x[i + 1].cast[out_dtype]() + (hu**2 - hd**2)*x[i].cast[out_dtype]() - hu**2 * x[i - 1].cast[out_dtype]())/ (hu * hd * (hu + hd))
        result[i] = fi
