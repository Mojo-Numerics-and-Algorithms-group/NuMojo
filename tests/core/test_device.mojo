...
def test_invalid_type_falls_back_to_cpu():
...
def test_gpu_empty_name_falls_back_to_cpu():
...
def test_cpu_with_nonzero_id_falls_back():
...
def test_cpu_with_name_falls_back():
...
def test_invalid_gpu_backend_falls_back():
...
def test_negative_gpu_id_falls_back():
...
def test_parse_cpu_string_variant():
    var d = Device("cpu:0")
...
def test_unchecked_init_arbitrary():
...
def test_available_devices_contains_cpu():
