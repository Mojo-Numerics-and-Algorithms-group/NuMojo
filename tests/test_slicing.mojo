...
def test_slicing():
    ...
    # Test basic indexing
    check_is_close(w[0,0,0], np_w[0,0,0], "3D array indexing (C-order) [0,0,0]")
    check_is_close(w[1,2,3], np_w[1,2,3], "3D array indexing (C-order) [1,2,3]")
    ...
    # Test basic indexing
    check_is_close(y[0,0,0], np_y[0,0,0], "3D array indexing (F-order) [0,0,0]")
    check_is_close(y[1,2,3], np_y[1,2,3], "3D array indexing (F-order) [1,2,3]")
